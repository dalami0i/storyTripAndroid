...
public class AroundDetailMapActivity extends AppCompatActivity implements
    ...
    AttrClient seletedAttr;//선택된 item의 공용 객체
...
//                Location mylocation = locationManager.getLastKnownLocation(LocationManager.GPS_PROVIDER);
        ...
        mMap.moveCamera(CameraUpdateFactory.newLatLng(new LatLng(Double.parseDouble(seletedAttr.mapy), Double.parseDouble(seletedAttr.mapx))));
...
//        Log.e("빌드에이피아이","비이이이이이틀");
    ...
    private void checkAround(GoogleMap mMap){
        ...
        switch (seletedAttr.categoryNum){
            case "1":
                mMap.addMarker(new MarkerOptions()
                        .position(new LatLng(Double.parseDouble(seletedAttr.mapy), Double.parseDouble(seletedAttr.mapx)))
                        .title(seletedAttr.title)
                        .icon(BitmapDescriptorFactory.fromResource(R.drawable.z_marker_activity)))
                        .setTag(seletedAttr.contentID);
                break;
            case "3":
                mMap.addMarker(new MarkerOptions()
                        .position(new LatLng(Double.parseDouble(seletedAttr.mapy), Double.parseDouble(seletedAttr.mapx)))
